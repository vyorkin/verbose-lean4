...
notation:50 f:80 " непрерывна в " x₀ => continuous_function_at f x₀
notation:50 u:80 " стремится к " l => sequence_tendsto u l

def increasing_seq (u : ℕ → ℝ) := ∀ n m, n ≤ m → u n ≤ u m

notation u " возрастает" => increasing_seq u

def is_supremum (M : ℝ) (u : ℕ → ℝ) :=
(∀ n, u n ≤ M) ∧ ∀ ε > 0, ∃ n₀, u n₀ ≥ M - ε

notation M " является супремумом " u => is_supremum M u
...
protected def Verbose.Russian.Subset (s₁ s₂ : Set α) :=
  ∀ x, x ∈ s₁ → x ∈ s₂

instance (priority := high) Verbose.Russian.hasSubset : HasSubset (Set α) :=
  ⟨Verbose.Russian.Subset⟩
